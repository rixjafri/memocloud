...
//                            SceneDelegate.sharedInstance.pushToLoginRoot(isAnimate: false)
    ...
    /*
    func call(target: TargetType, completion: @escaping (Result<CarListResponse, NetworkError>) -> Void) {
        provider.request(target as! Apis) { result in
            switch result {
            case .success(let response):
                print("Raw Response: \(String(data: response.data, encoding: .utf8) ?? "No Data")")
                do {
                    // Attempt to decode into CarListResponse
                    let detectionResponse = try JSONDecoder().decode(CarListResponse.self, from: response.data)
                    completion(.success(detectionResponse))
                } catch {
                    print("Decoding error: \(error)") // Optional: Log the error for debugging
                    completion(.failure(.decodingError))
    ...
    */
    ...
    func callV2<T: Codable>(target: TargetType, completion: @escaping (Result<T, NetworkError>) -> Void) {
        provider.request(target as! Apis) { result in
            switch result {
            case .success(let response):
                print("Raw Response: \(String(data: response.data, encoding: .utf8) ?? "No Data")")
